...
async def stage_4(message: types.Message, state: FSMContext,  session: AsyncSession):
    ...
    if current_state == FSM.achieve_goal_state:
        ...
        if not flag:
            ...
            #print(parametrs['sexxx'], parametrs['muscl'], parametrs['goal'], parametrs['detail_goal'])
            ...
            if not flag_print:
                for training in await orm_get_trainings(session):
                    ...
                    if parametrs['goal'] == training.energy_feelins and (
                        ...
                        flag_goal = True
                    flag_detailed = False
                    #print(training.detailed_muscle_group.split())
                    #print(parametrs['detail_goal'])
                    for muscle in training.detailed_muscle_group.split():
                        if muscle == parametrs['detail_goal']:
                            flag_detailed = True
                    #print(flag, flag_goal, flag_detailed)
                    if flag and flag_detailed:
                        await message.answer(str(training.link))
                        flag_print = True
                        k += 1
        if flag_print:
            await message.answer("Тренировка взята с каналов Nike Training и Jordan Yeoh Fitness")
